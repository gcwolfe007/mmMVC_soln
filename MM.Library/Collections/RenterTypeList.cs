...
{
     [Serializable()]
    public class RenterTypeList : NameValueListBase<int, string>
    {
         private static RenterTypeList _list;
        ...
        /// </summary>
        internal static void SetCache(RenterTypeList list)
        {
            _list = list;
        }
        ...
        public static void GetList(EventHandler<DataPortalResult<RenterTypeList>> callback)
        {
            if (_list == null)
                DataPortal.BeginFetch<RenterTypeList>((o, e) =>
                {
                    SetCache(e.Object);
                    callback(o, e);
                });
            else
                callback(null, new DataPortalResult<RenterTypeList>(_list, null, null));
        }
...
#else
        public static RenterTypeList GetList()
        {
            if (!IsCached)
                SetCache(DataPortal.Fetch<RenterTypeList>());
            return _list;
        }
