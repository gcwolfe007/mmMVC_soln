...
{
   public class SP_Get_ContactInfoType_List : MM_Stored_Procedure
   {
       ...
       public SP_Get_ContactInfoType_List(string requestedSP)
             : base(requestedSP)         
         {
       ...
       public spResult ExecuteSP() 
       {
           var myData = new DalManager();
             var myResult = new spR_Get_ContactInfoTypes_List_Result();
             ...
             try
             {
                 using (var myAccessCommand = new SqlCommand(_storedProcedureName, _mySQLConn))
                 {
                    ...
                    using (var dr = new SafeDataReader(myAccessCommand.ExecuteReader()))
                     {
                         var rdrResult = Mapper.Map<List<ContactInfoTypeDTO>>(dr);
       ...
       {
           // Parent - 
           Mapper.CreateMap<SafeDataReader, ContactInfoTypeDTO>()           
           ;
           // Child etc - 
       }
